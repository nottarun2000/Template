...
{
    public class gRPCClient : ClientCommunicateService, ICommunicateService
    {
        // Private fields for client configuration
        private gRPCClient _service { get; set; }
        ...
        // Constructor for initializing the gRPC client
        public gRPCClient(string host = "localhost", int port = 5000)
        {
            _host = host;
            _port = port;
            _service = this;
        ...
        // Method to send a message to the server with a specified message type
        public async Task SendMessageToServer(string message, ServerMessageType serverMessageType)
        {
            await _service.SendMessageAsync(message, serverMessageType.ToString());
        }
    }
}
